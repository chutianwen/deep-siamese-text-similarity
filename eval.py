...
tf.flags.DEFINE_string("checkpoint_dir", "runs/1548536327/checkpoints", "Checkpoint directory from training run")
tf.flags.DEFINE_string("eval_filepath", "validation.txt0", "Evaluate on this data (Default: None)")
tf.flags.DEFINE_string("vocab_filepath", "runs/1548536327/checkpoints/vocab", "Load training time vocabulary (Default: None)")
tf.flags.DEFINE_string("model", "runs/1548536327/checkpoints/model-59000", "Load trained model checkpoint (Default: None)")
	...
	with sess.as_default():
		...
		for db in batches:
			...
			batch_predictions, batch_acc, batch_sim = sess.run([predictions, accuracy, sim],
			                                                   {input_x1: x1_dev_b, input_x2: x2_dev_b, input_y: y_dev_b, dropout_keep_prob: 1.0})
